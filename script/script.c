...
int script_parser_fetch(char *main_name, char *sub_name, int value[], int count)
{
	...
	debug("enter script parse fetch. \n");
	//检查脚本buffer是否存在
	if(!script_mod_buf)
	{
		return SCRIPT_PARSER_EMPTY_BUFFER;
	...
	debug("gpio: main name is : %s, sub_name is: %s", main_char, sub_char);
	...
	for(i=0;i<script_main_key_count;i++)
	{
		...
		for(j=0;j<main_key->lenth;j++)
		{
			...
			debug("pattern is: 0x%x, word_count is: 0x%x, ", pattern, word_count);
			//取出数据
			switch(pattern)
			{
