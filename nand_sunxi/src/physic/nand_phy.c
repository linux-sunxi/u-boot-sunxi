...
__s32  PHY_BlockErase(struct __PhysicOpPara_t *pBlkAdr)
{
	...
	for(NandIndex = 0; NandIndex<CHANNEL_CNT;NandIndex++)
	{
		...
		for (i = 0; i < plane_cnt; i++){
			...
			//PRINT("PHY_BlockErase, Chaneel %d, Blcok 0x%x\n", block_in_chip+ i*MULTI_PLANE_BLOCK_OFFSET);
		}
...
__s32  PHY_PageRead(struct __PhysicOpPara_t *pPageAdr)
{
	...
	for(NandIndex = 0; NandIndex<CHANNEL_CNT;NandIndex++)
	{
		...
		for (i = 0; i < plane_cnt; i++){
			...
			readop.page = pPageAdr->PageNum;
				...
				bitmap_in_single_page = FULL_BITMAP_OF_SINGLE_PAGE &
					(pPageAdr->SectBitmap >> ((plane_cnt*NandIndex + i)*SECTOR_CNT_OF_SINGLE_PAGE));
				readop.sectorbitmap = bitmap_in_single_page;
	...
	if(badspare)
	{
		temp_oob == (__u8 *)(pPageAdr->SDataPtr);
		temp_oob[0] = 0;
	}
	...
	for(NandIndex = 0; NandIndex<CHANNEL_CNT;NandIndex++)
	{
		...
		for (i = 0; i < plane_cnt; i++){
			...
			writeop.page = pPageAdr->PageNum;
			    ...
			    writeop.mainbuf = (__u8 *)(pPageAdr->MDataPtr) + 512*SECTOR_CNT_OF_SINGLE_PAGE*(plane_cnt*NandIndex + i);
			    if (pPageAdr->SDataPtr)
