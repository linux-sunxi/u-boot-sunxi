...
__s32  PHY_PageRead(struct __PhysicOpPara_t *pPageAdr)
{
	...
	//DBUG_INF("  PR : block %x, page %x, data: %x \n", pPageAdr->BlkNum, pPageAdr->PageNum, *((__u32 *)pPageAdr->MDataPtr));
	...
	if (0xff == chip){
		PHY_ERR("PHY_PageRead : beyond chip count\n");
		PHY_ERR("PHY_PageRead : bank %x, block %x, page %x\n", pPageAdr->BankNum, pPageAdr->BlkNum, pPageAdr->PageNum);
		return -ERR_INVALIDPHYADDR;
	}
	/*get block no within chip*/
	block_in_chip = _cal_block_in_chip(pPageAdr->BankNum,pPageAdr->BlkNum);
...
void PHY_FreePageCheck(struct __PhysicOpPara_t  *pPageAdr)
{
	...
	for(i = 0; i < 8; i++)
	{
		if( (i%4 != 3) && (oob[i] != 0xff))
		{
			...
			PRINT("oob data : --%x--%x--%x--%x--%x--%x--%x--\n",
				oob[0],oob[1],oob[2],oob[4],oob[5],oob[6], oob[7]);
			//while(1);
		}
	}
...
__s32  PHY_PageWrite(struct __PhysicOpPara_t  *pPageAdr)
{
	...
	if (0xff == chip){
		...
		PHY_ERR("PHY_PageWrite : bank %x, block %x, page %x\n", pPageAdr->BankNum, pPageAdr->BlkNum, pPageAdr->PageNum);
		return -ERR_INVALIDPHYADDR;
	}
	/*get block no within chip*/
	block_in_chip = _cal_block_in_chip(pPageAdr->BankNum,pPageAdr->BlkNum);
...
__s32  PHY_PageCopyback(struct __PhysicOpPara_t *pSrcPage, struct __PhysicOpPara_t *pDstPage)
{
	...
	/*cant not use copyback*/
		...
		if (ret < 0){
		    PRINT("PHY_PageCopyback PageRead error! ret = 0x%x\n", ret);
...
PHY_PageCopyback_exit:
	if (ret == -ERR_TIMEOUT)
		...
		PHY_ERR("PHY_PageCopyback : copy back unknow error, ret = 0x%x\n", ret);
