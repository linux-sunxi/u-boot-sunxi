...
__u8 read_retry_default_val[2][4][READ_RETRY_MAX_REG_NUM];
...
void _dma_config_start(__u8 rw, __u32 attr, __u32 src_addr, __u32 dst_addr, __u32 len)
{
	__u32 reg_val;
	...
	if(rw)
		NFC_WRITE_REG(NFC_REG_MDMA_ADDR, src_addr);
	else
		NFC_WRITE_REG(NFC_REG_MDMA_ADDR, dst_addr);
...
__s32 NFC_Init(NFC_INIT_INFO *nand_info )
{
	...
	NandIOBase[0] = 0x01c03000;
	NandIOBase[1] = 0x01c05000;
    //--------------------------------------
