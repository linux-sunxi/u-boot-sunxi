...
#if defined(CONFIG_CMD_SAVER)
int do_serial_save_raw (cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	ulong offset = 0;
	ulong size   = 0;
	int cr = 0;
	...
	printf ("## Ready for raw upload, press ENTER to proceed ...\n");
	for (;;) {
		if (getc() == '\r')
			break;
	}
	while (size-- > 0) {
		uchar ch = *(volatile uchar*)offset++;
		putc(ch);
	}
	printf ("\n## Raw upload complete\n");
	return 0;
...
U_BOOT_CMD(
	saver, 3, 0,	do_save_serial_raw,
	"save raw file over serial line",
	"[ off ] [size]\n"
	"    - save S-Record file over serial line with offset 'off' and size 'size'"
);
#endif

#if defined(CONFIG_CMD_DUMPASCII)
int do_dumpascii (cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	ulong offset = 0;
	ulong size   = 0;
	int cr = 0;
