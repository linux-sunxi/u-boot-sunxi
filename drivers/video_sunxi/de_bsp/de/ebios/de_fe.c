...
//*********************************************************************************************
// function       : DE_SCAL_Agth_Config(__u8 sel, __scal_src_type_t *in_type,__scal_src_size_t *in_size,
//                                      __scal_out_size_t *out_size, __u8 dien,__u8 trden,__scal_3d_outmode_t outmode)
// description    : select sram mapping mothed and agorithm modify
// parameters     :
//                 sel <scaler select>
//                 in_size <scale region define,  src size, offset, scal size>
//                 in_type <src data type>
//                 out_size <scale out size, when output interlace, the height is 2*outheight ,for example 480i, the value is 480>
//                 dien <deinterlace enable>
                          ...
                          __u8 dien,__u8 trden,__scal_3d_outmode_t outmode)
{
	...
	else if((inwidth>2048) && (outwidth>2048))
	{
		...
		{
			scal_dev[sel]->bypass.bits.sram_map_sel = 1; //4096 mode
			linebuflen = 4096;
			cibuflen = 2048;
		}
		...
		{
			scal_dev[sel]->bypass.bits.sram_map_sel = 2; //2560 mode
			linebuflen = 2560;
			cibuflen = 0;
		}
	}
	...
	{
		scal_dev[sel]->bypass.bits.sram_map_sel = 0; //normal mode
		linebuflen = 2048;
...
// return         : 
//               success
// history        : 2012/06/05 added alpha coef select, modify alpha coef in future according to application
//***********************************************************************************************                                        
                               ...
                               __scal_out_size_t *out_size, __scal_out_type_t *out_type, __u8 smth_mode)  
{
    ...
    __u32 ch0v_fir_coef_addr, ch0h_fir_coef_addr, ch1v_fir_coef_addr, ch1h_fir_coef_addr, ch3h_fir_coef_addr;
    __u32 ch0v_fir_coef_ofst, ch0h_fir_coef_ofst, ch1v_fir_coef_ofst, ch1h_fir_coef_ofst, ch3v_fir_coef_addr;
    __s32 fir_ofst_tmp;
    __u32 i;
...
// note             : alpha can only be enable when input fmt interleaved and ouput fmt argb
//*********************************************************************************************** 
...
// return         : 
//                 success
// note           : flag out is enabled in default, so current flag address must be set before
//*********************************************************************************************** 
__s32 DE_SCAL_Set_Di_Ctrl(__u8 sel, __u8 en, __u8 mode, __u8 diagintp_en, __u8 tempdiff_en)
{
	scal_dev[sel]->di_ctrl.bits.en = en;
	scal_dev[sel]->di_ctrl.bits.flag_out_en = 0x0;  //for flag out, default enable
...
//*********************************************************************************************
// function       : DE_SCAL_Set_Di_PreFrame_Addr(__u8 sel, __u32 luma_addr, __u32 chroma_addr)
// description    : set scaler deinterlace pre frame luma/chroma address
// parameters     :
//                 sel <scaler select>, 
...
//*********************************************************************************************
// function       : DE_SCAL_Set_Di_MafFlag_Src(__u8 sel, __u32 cur_addr, __u32 pre_addr, __u32 stride)
...
__s32 DE_SCAL_Output_Select(__u8 sel, __u8 out)
{
    ...
    else if(out < 3)
    {
        ...
        scal_dev[sel]->frm_ctrl.bits.out_port_sel = 0x0;  //only to be0
    }
...
//*********************************************************************************************
// function       : DE_SCAL_Set_3D_Di_PreFrame_Addr(__u8 sel, __scal_buf_addr_t *addr, __scal_buf_addr_t *addrtrd)
// description    : set scaler deinterlace pre frame luma/chroma address
// parameters     :
//                 sel <scaler select>, 
//                 addr  <pre frame buffer address for 3 channel, 32 bit absolute address>
//                 addrtrd <pre 3D source right image buffer address, only needed when 3dinmode is FP>
// return         : 
//                 success
//*********************************************************************************************** 
...
//*********************************************************************************************
// function       : DE_SCAL_Set_Writeback_Addr_ex(__u32 sel, __scal_buf_addr_t *addr, __scal_out_size_t *size,
